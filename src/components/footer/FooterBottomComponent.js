...
function FooterBottomComponent() {
  const pageTitles = ["Bizni ijtimoiy tarmoqlarda kuzating", "mobil ilovalar"];
      ...
      <div className="container">
        <div className={style.content}>
          <div className={style.div}>
            ...
            <ul className={style.imgWrapper}>
              {socials.map((item, idx) => {
                return (
                  <li className={style.socials} key={idx}>
                    {/* <Image src={item.img} alt="image" /> */}
                    <a href={item.link} target='_blank' rel="noreferrer" className={item.name}></a>
                  </li>
                );
              })}
            </ul>
